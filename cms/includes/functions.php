	...
	*/
	...
	function navegacion($seccion_seleccionada, $contenido_seleccionado){
		...
		while($seccion = mysql_fetch_array($grupo_secciones)){
			...
			if(in_array('PUBLICACIONES', $titulos)){
    			echo "<a href=\"publicaciones.php\">{$seccion["titulo"]}</a></li><br />";
			}elseif(in_array('ARTÌCULOS', $titulos)) {
				echo "<a href=\"modulos.php\">{$seccion["titulo"]}</a></li><br />";
			}elseif(in_array('CONTACTO', $titulos)){
				//no mostrar nada
			}else{
				echo "<a href=\"editar-seccion.php?seccion=" . urlencode($seccion["id"]) . "\">{$seccion["titulo"]}</a></li><br />";
			}
		}
		
		echo "<a href=\"noticias.php\">NOTICIAS</a><br />";
		echo "<a href=\"contactos.php\">Base de Datos de Contactos</a>";
	}
